...
object WsManager {
    ...
    fun searchSound(project: String, query: String, filename: String = "") =
        send(JSONObject().put("type","search_sound").put("project",project).put("query",query).put("filename",filename))
    fun uploadFile(project: String, filename: String, dataB64: String) =
        send(JSONObject().put("type","upload_file").put("project",project).put("filename",filename).put("data",dataB64))
    ...
    fun setupFirebase(token: String, project: String, pkg: String) =
        send(JSONObject().put("type","setup_firebase").put("token",token).put("project",project).put("pkg",pkg))
