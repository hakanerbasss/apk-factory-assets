...
import com.wizaicorp.apkfactory.showAdAndRun
import androidx.compose.ui.platform.LocalContext
...
// ── Küresel API Durum Yöneticisi ─────────────────────────────
object GlobalApiState {
    var apis by androidx.compose.runtime.mutableStateOf(listOf<com.wizaicorp.apkfactory.data.ApiInfo>())
    var defProv by androidx.compose.runtime.mutableStateOf("Claude")
...
fun executeWithApiCheck(action: () -> Unit) {
    val jKey = GlobalApiState.apis.find { it.name.equals(GlobalApiState.defProv, ignoreCase = true) }?.hasKey == true
    val sKey = if (GlobalApiState.seniorProv.isNotEmpty() && GlobalApiState.seniorProv != "Devre Dışı") {
        GlobalApiState.apis.find { it.name.equals(GlobalApiState.seniorProv, ignoreCase = true) }?.hasKey == true
    } else true

    if (!jKey) {
        GlobalApiState.warningType = "junior"
        GlobalApiState.pendingAction = action
    } else if (!sKey) {
        GlobalApiState.warningType = "senior"
        GlobalApiState.pendingAction = action
    } else {
        action()
    }
        ...
        WsManager.events.collect { event ->
            when (event) {
                ...
                is WsEvent.TaskDone -> {
                    ...
                    if (event.success) {
                        // IDLE yapma — önce NextTask kontrol et, zincir yoksa IDLE ol
                ...
                is WsEvent.ProjectDone -> {
                    ...
                    if (event.success && pendingNewProjectTask.isNotEmpty()) {
                        ...
                        // Yeni proje seçili olsun — Yedekle/Geri Al butonları görünsün
                        val newProj = projects.find { it.name == event.name }
                            ?: ProjectInfo(event.name, "")
                ...
                is WsEvent.Projects    -> {
                    projects = event.list
                    // selectedProject null ama ismi biliyorsak güncelle
                    val cur = selectedProject
                    if (cur != null) {
                        val fresh = event.list.find { it.name == cur.name }
                        if (fresh != null) selectedProject = fresh
                    }
                }
                is WsEvent.ApiList -> GlobalApiState.apis = event.list
                is WsEvent.Settings -> {
                    GlobalApiState.defProv = event.data["DEFAULT_PROVIDER"] ?: "Claude"
...
FloatingActionButton(
    onClick = { 
        showAdAndRun(context) {
            com.wizaicorp.apkfactory.showAdAndRun(context) { showNewProject = true } 
        }
    },
        ...
        Box(modifier = Modifier.fillMaxSize().padding(padding).imePadding()) {
            when (activeTab) {
                AppTab.PROJECTS -> ProjectsTab(
                    ...
                    onAdmob = { p, appId, unitId ->
                        selectedProject = p
                        logs = listOf("💰 AdMob ekleniyor: ${p.name}...")
                        runMode = RunMode.AUTOFIXING
                        WsManager.addAdmob(p.name, appId, unitId)
                        activeTab = AppTab.AUTOFIX
                    },
                ...
                AppTab.AUTOFIX -> AutoFixTab(
                    ...
                    onTask = { p, task ->      // prj e "task" — görev ver
                        ...
                        runMode = RunMode.AUTOFIXING; newApkPath = ""
                        // --- OTONOM FIREBASE (gorev) ---
                        val _fbKw2 = listOf("firebase","firestore","database","auth","login","giris","kayit","chat","mesaj","storage","realtime","notification","bildirim")
                        val _fbPr2 = context.getSharedPreferences("apkfactory_prefs", android.content.Context.MODE_PRIVATE)
                        if (_fbPr2.getBoolean("firebase_connected", false) && _fbKw2.any { k -> task.lowercase().contains(k) }) {
                            scope.launch {
                                val _a2 = com.google.android.gms.auth.api.signin.GoogleSignIn.getLastSignedInAccount(context)
                                if (_a2 != null) {
                                    val _t2 = com.wizaicorp.apkfactory.FirebaseAuthManager.getFirebaseAccessToken(context, _a2)
                                    if (_t2 != null) { WsManager.setupFirebase(_t2, p.name, p.packageName) }
                                }
                            }
                        }
                        // --------------------------------
                    ...
                    onRestoreAndStop = { promptType = PromptType.NONE; promptErrorMsg = ""; autoMode = false; WsManager.restoreAgentBackups() },
    ...
    if (GlobalApiState.warningType.isNotEmpty()) {
        AlertDialog(
        ...
        NewProjectDialog(onDismiss = { showNewProject = false },
            onCreate = { name, task, pkg ->
                executeWithApiCheck {
                    ...
                    // --- OTONOM FIREBASE (yeni proje) ---
                    val _fbKw = listOf("firebase","firestore","database","auth","login","giris","kayit","chat","mesaj","storage","realtime","notification","bildirim")
                    val _fbPr = context.getSharedPreferences("apkfactory_prefs", android.content.Context.MODE_PRIVATE)
                    if (_fbPr.getBoolean("firebase_connected", false) && _fbKw.any { kw -> task.lowercase().contains(kw) }) {
                        scope.launch {
                            val _acc = com.google.android.gms.auth.api.signin.GoogleSignIn.getLastSignedInAccount(context)
                            if (_acc != null) {
                                val _tk = com.wizaicorp.apkfactory.FirebaseAuthManager.getFirebaseAccessToken(context, _acc)
                                if (_tk != null) {
                                    val _pkg = pkg.ifBlank { "com.wizaicorp." + name.replace("-","_").lowercase() }
                                    WsManager.setupFirebase(_tk, name, _pkg)
                                }
                            }
                        }
                    }
...
fun ProjectsTab(
    ...
    onAdmob: (ProjectInfo, String, String) -> Unit = { _, _, _ -> },
    ...
    var soundTargetProject by remember { mutableStateOf<ProjectInfo?>(null) }
    var firebaseTargetProject by remember { mutableStateOf<ProjectInfo?>(null) }
    ...
    // Firebase google-services.json file picker
    val firebaseLauncher = androidx.activity.compose.rememberLauncherForActivityResult(
        androidx.activity.result.contract.ActivityResultContracts.GetContent()
    ) { uri ->
        uri?.let {
            val p = firebaseTargetProject ?: return@let
            try {
                ...
                if (bytes.isNotEmpty()) {
                    ...
                    WsManager.uploadFile(p.name, "google-services.json", b64)
                    firebaseTargetProject = null
                }
            } catch (e: Exception) {
                android.widget.Toast.makeText(context, "Hata: ${e.message}", android.widget.Toast.LENGTH_LONG).show()
            }
                            ...
                            Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                                ...
                                Box {
                                    ...
                                    DropdownMenu(expanded = showMenu, onDismissRequest = { showMenu = false }, modifier = Modifier.background(CARD)) {
                                        ...
                                        DropdownMenuItem(text = { Text("🔊 Ses Ekle", color = WHITE, fontSize = 13.sp) }, onClick = { showMenu = false; soundTargetProject = p })
                                        DropdownMenuItem(text = { Text("🔥 Firebase Ekle", color = WHITE, fontSize = 13.sp) }, onClick = { showMenu = false; firebaseTargetProject = p })
        ...
        AlertDialog(
            ...
            confirmButton = {
                Button(onClick = {
                    ...
                    onAdmob(p, finalAppId, finalUnitId)
    ...
    // === Ses Ekle Dialog ===
    soundTargetProject?.let { p ->
        var soundQuery by remember { mutableStateOf("") }
        var soundFilename by remember { mutableStateOf("") }
        val soundUriHandler = androidx.compose.ui.platform.LocalUriHandler.current
        AlertDialog(
            onDismissRequest = { soundTargetProject = null },
            containerColor = CARD, shape = RoundedCornerShape(16.dp),
            title = { Text("🔊 Ses Ekle (Freesound)", color = WHITE, fontWeight = FontWeight.Bold) },
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(10.dp),
                    modifier = Modifier.verticalScroll(rememberScrollState())) {
                    Text("Freesound.org kütüphanesinden ücretsiz ses efekti arayıp projenin res/raw/ klasörüne indirir.",
                        color = GREY, fontSize = 12.sp)

                    Card(colors = CardDefaults.cardColors(containerColor = Color(0xFF1A2A1A)),
                        border = BorderStroke(1.dp, GREEN.copy(alpha = 0.3f)), shape = RoundedCornerShape(10.dp)) {
                        Column(modifier = Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                            Text("ℹ️ Nasıl Çalışır?", color = GREEN, fontSize = 12.sp, fontWeight = FontWeight.Bold)
                            Text("1. Aşağıya İngilizce arama yaz (button click, whoosh, explosion...)", color = WHITE, fontSize = 11.sp)
                            Text("2. İlk eşleşen ses otomatik indirilir → res/raw/", color = WHITE, fontSize = 11.sp)
                            Text("3. Kodda R.raw.dosya_adi ile kullanırsın", color = WHITE, fontSize = 11.sp)
                            Spacer(Modifier.height(4.dp))
                            Text("⚠️ Önce API Key al:", color = ORANGE, fontSize = 11.sp, fontWeight = FontWeight.Medium)
                            Text("🌐 freesound.org/apiv2/apply/", color = ACCENT, fontSize = 11.sp,
                                modifier = Modifier.clickable { soundUriHandler.openUri("https://freesound.org/apiv2/apply/") })
                            Text("Aldıktan sonra: Ayarlar → Freesound API Key", color = GREY, fontSize = 10.sp)
                        }
                    }

                    OutlinedTextField(value = soundQuery, onValueChange = { soundQuery = it },
                        label = { Text("🔍 Ses Ara (İngilizce)", fontSize = 11.sp) },
                        placeholder = { Text("button click, notification, whoosh...", fontSize = 10.sp, color = GREY) },
                        singleLine = true, modifier = Modifier.fillMaxWidth(),
                        shape = RoundedCornerShape(8.dp),
                        colors = OutlinedTextFieldDefaults.colors(focusedBorderColor = GREEN,
                            ...
                            unfocusedTextColor = WHITE, focusedContainerColor = SURFACE,
                            unfocusedContainerColor = SURFACE))
                    OutlinedTextField(value = soundFilename, onValueChange = { soundFilename = it },
                        label = { Text("Dosya adı (opsiyonel)", fontSize = 11.sp) },
                        placeholder = { Text("örn: btn_click  (boş bırakırsan otomatik)", fontSize = 10.sp, color = GREY) },
                        singleLine = true, modifier = Modifier.fillMaxWidth(),
                        shape = RoundedCornerShape(8.dp),
                        colors = OutlinedTextFieldDefaults.colors(focusedBorderColor = GREEN,
                            unfocusedBorderColor = BORDER, focusedTextColor = WHITE,
                    ...
                    Card(colors = CardDefaults.cardColors(containerColor = SURFACE),
                        ...
                        Text("🤖 İpucu: Görev modunda AI'a “buton tıklama sesi ekle” gibi bir görev verirsen, AI kodu yazar. Sonra buradan sesi indirip projeye eklersin.",
                            modifier = Modifier.padding(8.dp), color = GREY, fontSize = 10.sp, lineHeight = 14.sp)
                    }
                }
            },
            confirmButton = {
                Button(onClick = {
                    if (soundQuery.isNotBlank()) {
                        WsManager.searchSound(p.name, soundQuery, soundFilename)
                        soundTargetProject = null
                    }
                }, enabled = soundQuery.isNotBlank(),
                    colors = ButtonDefaults.buttonColors(containerColor = GREEN),
                    shape = RoundedCornerShape(10.dp)) { Text("🔊 Ara & İndir") }
            },
            dismissButton = { TextButton(onClick = { soundTargetProject = null }) { Text("İptal", color = GREY) } }
        )
    }

    // === Firebase Ekle Dialog ===
    firebaseTargetProject?.let { p ->
        val fbUriHandler = androidx.compose.ui.platform.LocalUriHandler.current
        AlertDialog(
            onDismissRequest = { firebaseTargetProject = null },
            containerColor = CARD, shape = RoundedCornerShape(16.dp),
            title = { Text("🔥 Firebase Ekle", color = WHITE, fontWeight = FontWeight.Bold) },
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(10.dp),
                    modifier = Modifier.verticalScroll(rememberScrollState())) {
                    Text("Firebase Console'dan aldığın google-services.json dosyasını seç. Gerisini biz hallederiz.",
                        color = GREY, fontSize = 12.sp)

                    Card(colors = CardDefaults.cardColors(containerColor = Color(0xFF2A1A0A)),
                        border = BorderStroke(1.dp, Color(0xFFFF9800).copy(alpha = 0.3f)), shape = RoundedCornerShape(10.dp)) {
                        Column(modifier = Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                            Text("📋 Önce Firebase'da Yap:", color = Color(0xFFFF9800), fontSize = 12.sp, fontWeight = FontWeight.Bold)
                            Text("1. Firebase Console'a git → Yeni proje oluştur", color = WHITE, fontSize = 11.sp)
                            Text("2. Android uygulaması ekle", color = WHITE, fontSize = 11.sp)
                            Text("3. Paket adını gir: ${p.packageName}", color = ACCENT, fontSize = 11.sp, fontWeight = FontWeight.Medium)
                            Text("4. google-services.json'ı indir", color = WHITE, fontSize = 11.sp)
                            Spacer(Modifier.height(4.dp))
                            Text("🌐 Firebase Console'u Aç", color = ACCENT, fontSize = 12.sp,
                                fontWeight = FontWeight.Medium,
                                modifier = Modifier.clickable { fbUriHandler.openUri("https://console.firebase.google.com/") }
                                    .padding(vertical = 4.dp))
                        }
                    }

                    Card(colors = CardDefaults.cardColors(containerColor = Color(0xFF0A1A2A)),
                        border = BorderStroke(1.dp, ACCENT.copy(alpha = 0.3f)), shape = RoundedCornerShape(10.dp)) {
                        Column(modifier = Modifier.padding(10.dp), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                            Text("⚙️ Dosya Seçince Otomatik Yapılacaklar:", color = ACCENT, fontSize = 12.sp, fontWeight = FontWeight.Bold)
                            Text("✅ google-services.json → app/ klasörüne kopyalanır", color = GREEN, fontSize = 11.sp)
                            Text("✅ build.gradle → google-services plugin eklenir", color = GREEN, fontSize = 11.sp)
                            Text("✅ Firebase BOM 32.7.0 eklenir", color = GREEN, fontSize = 11.sp)
                            Text("✅ Firestore + Auth + Storage bağımlılıkları eklenir", color = GREEN, fontSize = 11.sp)
                        }
                    }

                    Text("⚠️ Paket adı Firebase'da ve projede aynı olmalı!", color = ORANGE, fontSize = 11.sp)
                }
            },
            confirmButton = {
                Button(onClick = { firebaseLauncher.launch("*/*") },
                    colors = ButtonDefaults.buttonColors(containerColor = Color(0xFFFF9800)),
                    shape = RoundedCornerShape(10.dp)) { Text("📂 google-services.json Seç") }
            },
            dismissButton = { TextButton(onClick = { firebaseTargetProject = null }) { Text("İptal", color = GREY) } }
        )
    }
...
fun AutoFixTab(
    ...
    if (showExportDialog && selectedProject != null) {
        AlertDialog(
            onDismissRequest = { showExportDialog = false },
            containerColor = CARD, shape = RoundedCornerShape(16.dp),
            title = { Text("✨ Kodları İndir", color = ACCENT, fontWeight = FontWeight.Bold, fontSize = 15.sp) },
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Text(
                        "Proje: ${selectedProject.name}\nBinary ve 100KB+ dosyalar hariç.",
                        color = GREY, fontSize = 12.sp
                    )
                    Text("Format seçin:", color = WHITE, fontSize = 13.sp)
                }
            },
            confirmButton = {
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(
                        onClick = {
                            ...
                            com.wizaicorp.apkfactory.data.WsManager.exportSources(selectedProject.name, "txt")
                            android.widget.Toast.makeText(ctx, "📄 TXT hazırlanıyor...", android.widget.Toast.LENGTH_SHORT).show()
                        },
                        colors = ButtonDefaults.buttonColors(containerColor = ACCENT)
                    ) { Text("📄 TXT") }
                    Button(
                        onClick = {
                            showExportDialog = false
                            com.wizaicorp.apkfactory.data.WsManager.exportSources(selectedProject.name, "zip")
                            android.widget.Toast.makeText(ctx, "📦 ZIP hazırlanıyor...", android.widget.Toast.LENGTH_SHORT).show()
                        },
            ...
            dismissButton = {
                TextButton(onClick = { showExportDialog = false }) { Text("İptal", color = GREY) }
            }
        )
    }
    ...
    if (GlobalApiState.showApiDialog) {
        AlertDialog(
            onDismissRequest = { GlobalApiState.showApiDialog = false },
            containerColor = CARD, shape = RoundedCornerShape(16.dp),
            title = { Text("🤖 Aktif API Durumu", color = WHITE, fontWeight = FontWeight.Bold, fontSize = 15.sp) },
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    Text("Şu an devrede olan yapay zeka modelleri aşağıdadır. Değişiklik yapmak için Ayarlar sekmesine gidebilirsiniz.", color = GREY, fontSize = 12.sp)
                    Card(colors = CardDefaults.cardColors(containerColor = SURFACE), border = BorderStroke(1.dp, ACCENT.copy(alpha = 0.5f)), modifier = Modifier.fillMaxWidth()) {
                        Column(modifier = Modifier.padding(10.dp)) {
                            Text("Ana Yapay Zeka (Junior)", color = ACCENT, fontSize = 11.sp, fontWeight = FontWeight.Bold)
                            Spacer(Modifier.height(4.dp))
                            Text("${GlobalApiState.defProv} - ${GlobalApiState.defModel.ifEmpty { "Seçilmedi" }}", color = WHITE, fontSize = 12.sp)
                        }
                    }
                    Card(colors = CardDefaults.cardColors(containerColor = SURFACE), border = BorderStroke(1.dp, ORANGE.copy(alpha = 0.5f)), modifier = Modifier.fillMaxWidth()) {
                        Column(modifier = Modifier.padding(10.dp)) {
                            Text("Gözlemci (Senior)", color = ORANGE, fontSize = 11.sp, fontWeight = FontWeight.Bold)
                            Spacer(Modifier.height(4.dp))
            ...
            confirmButton = {
                Button(onClick = { GlobalApiState.showApiDialog = false }, colors = ButtonDefaults.buttonColors(containerColor = ACCENT)) { Text("Kapat") }
            }
        )
    }
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        if (runMode == RunMode.IDLE && selectedProject != null && chainTask.isNotEmpty()) {
            ...
            Card(modifier = Modifier.fillMaxWidth().padding(horizontal = 10.dp, vertical = 6.dp),
                ...
                Column(modifier = Modifier.padding(10.dp)) {
                    ...
                    Button(onClick = {
                        executeWithApiCheck {
                            onTask(selectedProject, editedChainTask)
                            onDeleteChainTask()
                        }
                    },
        ...
        Row(
            ...
            // 1. SOL TARAF: Panik Butonu + Robot API Bilgi Butonu
            Row(verticalAlignment = Alignment.CenterVertically) {
                IconButton(onClick = { GlobalApiState.showApiDialog = true }, modifier = Modifier.size(32.dp)) {
                    Text("🤖", fontSize = 16.sp)
                }
            ...
            if (logs.isNotEmpty()) {
                Row(
                    ...
                    TextButton(
                        ...
                        Icon(Icons.Default.Folder, null, tint = ACCENT, modifier = Modifier.size(12.dp))
                        Spacer(Modifier.width(3.dp))
                        Text("Kodları İndir", color = ACCENT, fontSize = 10.sp)
                    }
                }
            }
        }
        ...
        // ── Alt buton alanı ──────────────────────────────────────────────────
        // Çalışırken: Durdur
        AnimatedVisibility(visible = runMode != RunMode.IDLE && promptType == PromptType.NONE) {
            Row(modifier = Modifier.fillMaxWidth().background(SURFACE)
        ...
        // Idle + log var → Yedekle + Temizle + Durdur
        AnimatedVisibility(visible = runMode == RunMode.IDLE && logs.isNotEmpty() && promptType == PromptType.NONE) {
            Row(modifier = Modifier.fillMaxWidth().background(SURFACE)
                .padding(horizontal = 10.dp, vertical = 7.dp),
                horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                if (selectedProject != null) {
                    OutlinedButton(onClick = { WsManager.restoreAgentBackups() },
                        modifier = Modifier.weight(1f), shape = RoundedCornerShape(10.dp),
                        border = BorderStroke(1.dp, RED),
                        contentPadding = PaddingValues(0.dp),
                        colors = ButtonDefaults.outlinedButtonColors(contentColor = RED)) {
                        Icon(Icons.Default.Undo, null, modifier = Modifier.size(13.dp))
                        Spacer(Modifier.width(4.dp))
                        Text("Geri Al", fontSize = 11.sp)
                    }
                ...
                OutlinedButton(onClick = onClearLogs,
                    ...
                    Icon(Icons.Default.ClearAll, null, modifier = Modifier.size(13.dp))
                    Spacer(Modifier.width(4.dp))
...
fun SettingsTab() {
    ...
    var freesoundKey by remember { mutableStateOf("") }
    ...
    LaunchedEffect(Unit) {
        WsManager.events.collect { event ->
            when (event) {
                ...
                is WsEvent.TaskDone     -> {
                    if (event.text.isNotEmpty()) toast = event.text
                    if (event.success && (event.text.contains("yedek") || event.text.contains("Yedek") || event.text.contains("dönüldü"))) {
                        WsManager.listBackups()
                    }
                }
                ...
                is WsEvent.Settings     -> {
                    ...
                    freesoundKey = event.data["FREESOUND_KEY"] ?: ""
                }
                else -> {}
            }
        }
    ...
    Column(modifier = Modifier.fillMaxSize().verticalScroll(rememberScrollState())) {
        ...
        SettingsSection("🤖","AutoFix","Model, deneme, sınırlar, şifre",
            ...
            Column(modifier = Modifier.padding(horizontal = 14.dp, vertical = 10.dp),
                ...
                Spacer(Modifier.height(8.dp))
                Text("🔊 Freesound API Key", fontSize = 12.sp, color = GREY)
                Text("freesound.org/apiv2/apply/ adresinden alın", fontSize = 10.sp, color = GREY.copy(alpha = 0.6f))
                Spacer(Modifier.height(4.dp))
                OutlinedTextField(value = freesoundKey, onValueChange = { freesoundKey = it },
                    placeholder = { Text("API Key...", fontSize = 12.sp) },
                    modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(10.dp), singleLine = true,
                    colors = OutlinedTextFieldDefaults.colors(focusedBorderColor = GREEN, unfocusedBorderColor = BORDER,
                        focusedTextColor = WHITE, unfocusedTextColor = WHITE, cursorColor = GREEN,
                        focusedContainerColor = CARD, unfocusedContainerColor = CARD),
                    textStyle = androidx.compose.ui.text.TextStyle(fontSize = 13.sp))
                ...
                Button(onClick = { WsManager.saveSettings(mapOf("DEFAULT_PROVIDER" to defProv,
                    "DEFAULT_MODEL" to selModel, "MAX_LOOPS" to maxLoops, "MAX_TOKENS" to maxTokens, "MAX_CHARS" to maxChars, "KEYSTORE_PASS" to ksPass, "SENIOR_PROVIDER" to seniorProv, "SENIOR_MODEL" to seniorModel, "UIX_PROVIDER" to uixProv, "UIX_MODEL" to uixModel, "FREESOUND_KEY" to freesoundKey)) },
                    modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(10.dp),
                    colors = ButtonDefaults.buttonColors(containerColor = ACCENT)) {
            ...
            groupedBackups.forEach { (projName, list) ->
                ...
                list.forEach { b ->
                    ...
                    Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 14.dp, vertical = 7.dp),
                        ...
                        IconButton(onClick = { WsManager.deleteBackup(b); toast = "🗑 Siliniyor..."; scope.launch { kotlinx.coroutines.delay(1000); WsManager.listBackups() } }, modifier = Modifier.size(34.dp)) {
                            Icon(Icons.Default.Delete, null, tint = RED.copy(alpha = 0.7f), modifier = Modifier.size(17.dp))
                        }
                    }
        ...
        Spacer(Modifier.height(8.dp))
        FirebaseAuthSection(context = context)
        ...
        var keystoreSearch by remember { mutableStateOf("") }
        val filteredKeystores = keystores.filter { keystoreSearch.isBlank() || it.name.contains(keystoreSearch, ignoreCase = true) || it.project.contains(keystoreSearch, ignoreCase = true) }
        ...
        SettingsSection("🔐","Keystore'lar","${keystores.size} keystore",
            ...
            Column(modifier = Modifier.padding(horizontal = 14.dp, vertical = 8.dp)) {
                OutlinedTextField(value = keystoreSearch, onValueChange = { keystoreSearch = it },
                    placeholder = { Text("Keystore ara...", fontSize = 12.sp) },
                    modifier = Modifier.fillMaxWidth(), shape = RoundedCornerShape(10.dp), singleLine = true,
                    leadingIcon = { Icon(Icons.Default.Search, null, tint = GREY, modifier = Modifier.size(18.dp)) },
                    trailingIcon = { 
                        Row(verticalAlignment = Alignment.CenterVertically) {
                            if (keystoreSearch.isNotEmpty()) {
                                IconButton(onClick = { keystoreSearch = "" }) { Icon(Icons.Default.Clear, null, tint = GREY, modifier = Modifier.size(16.dp)) }
                            }
                            IconButton(onClick = { WsManager.listKeystores() }) { Icon(Icons.Default.Refresh, null, tint = ACCENT, modifier = Modifier.size(18.dp)) }
                        }
                    },
                    colors = OutlinedTextFieldDefaults.colors(focusedBorderColor = ACCENT, unfocusedBorderColor = BORDER,
                        focusedTextColor = WHITE, unfocusedTextColor = WHITE, cursorColor = ACCENT,
            ...
            filteredKeystores.forEach { ks ->
                Row(modifier = Modifier.fillMaxWidth().padding(horizontal = 14.dp, vertical = 8.dp),
                    ...
                    Spacer(Modifier.width(8.dp))
                    IconButton(onClick = { WsManager.deleteKeystore(ks.name); toast = "🗑 Siliniyor..."; scope.launch { kotlinx.coroutines.delay(1000); WsManager.listKeystores() } }, modifier = Modifier.size(34.dp)) {
                        Icon(Icons.Default.Delete, null, tint = RED.copy(alpha = 0.7f), modifier = Modifier.size(17.dp))
                    }
                }
        ...
        androidx.compose.material3.Card(
            ...
            border = androidx.compose.foundation.BorderStroke(1.dp, BORDER), shape = androidx.compose.foundation.shape.RoundedCornerShape(12.dp)
        ) {
            Row(modifier = Modifier.padding(14.dp), verticalAlignment = Alignment.CenterVertically) {
                Text("🛠️", fontSize = 18.sp)
                Spacer(Modifier.width(10.dp))
                Column(modifier = Modifier.weight(1f)) {
                    Text("Geliştirici Modu", fontWeight = androidx.compose.ui.text.font.FontWeight.Bold, fontSize = 13.sp, color = WHITE)
                    Text("Sistem dosyalarını (termux-otonom-sistem) aç", fontSize = 11.sp, color = GREY)
                }
                Spacer(Modifier.width(8.dp))
                androidx.compose.material3.Surface(
                    onClick = {
                        try {
                            // Android'in dosya yolu kısıtlamasını geçici olarak kör ediyoruz (Bypass)
                            android.os.StrictMode.setVmPolicy(android.os.StrictMode.VmPolicy.Builder().build())
                            
                            val uri = android.net.Uri.parse("file:///storage/emulated/0/termux-otonom-sistem")
                            var intent = android.content.Intent(android.content.Intent.ACTION_VIEW).apply {
                                setDataAndType(uri, "resource/folder")
                                addFlags(android.content.Intent.FLAG_ACTIVITY_NEW_TASK)
                            }
                            
                            // Eğer cihaz 'resource/folder' tipini tanımazsa '*/*' ile zorla
                            ...
                            toast = "Dosya yöneticisi açılamadı!"
                        }
                    },
                    ...
                    Text("Klasörü Aç", modifier = Modifier.padding(horizontal = 10.dp, vertical = 6.dp),
                        color = PURPLE, fontSize = 11.sp, fontWeight = androidx.compose.ui.text.font.FontWeight.Bold)
                }
            }
        }
...
@Composable
fun FirebaseAuthSection(context: android.content.Context) {
    val scope = rememberCoroutineScope()
    val prefs = context.getSharedPreferences("apkfactory_prefs", android.content.Context.MODE_PRIVATE)
    var isFirebaseConnected by remember { mutableStateOf(prefs.getBoolean("firebase_connected", false)) }
    var status by remember { mutableStateOf("") }

    val firebaseAuthLauncher = androidx.activity.compose.rememberLauncherForActivityResult(
        ...
        if (result.resultCode == android.app.Activity.RESULT_OK) {
            ...
            task.addOnSuccessListener { account ->
                scope.launch {
                    ...
                    val token = com.wizaicorp.apkfactory.FirebaseAuthManager.getFirebaseAccessToken(context, account)
                    if (token != null) {
                        prefs.edit().putBoolean("firebase_connected", true).apply()
                        isFirebaseConnected = true
                        status = "✅ Firebase Otonom Bağlandı!"
                    } else {
                        status = "❌ Token alınamadı"
                    }
                }
            }.addOnFailureListener {
                status = "❌ Giriş iptal edildi"
            }
        }
    }
    ...
    androidx.compose.material3.Card(
        modifier = Modifier.fillMaxWidth().padding(horizontal = 12.dp),
        colors = androidx.compose.material3.CardDefaults.cardColors(containerColor = CARD),
        border = androidx.compose.foundation.BorderStroke(1.dp, BORDER), shape = androidx.compose.foundation.shape.RoundedCornerShape(14.dp)
    ) {
        Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                Text("🔥", fontSize = 18.sp)
                Spacer(Modifier.width(10.dp))
                Column(modifier = Modifier.weight(1f)) {
                    Text("Otonom Firebase", fontWeight = androidx.compose.ui.text.font.FontWeight.SemiBold, fontSize = 14.sp, color = WHITE)
                    Text("Yapay zekanın kendi başına Firebase projesi kurması için yetki ver", fontSize = 11.sp, color = GREY)
                }
            }
            
            if (status.isNotEmpty()) {
                Text(status, color = if (status.startsWith("✅")) GREEN else if (status.startsWith("❌")) RED else ORANGE, fontSize = 11.sp)
            }
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                if (!isFirebaseConnected) {
                    androidx.compose.material3.Button(
                        onClick = {
                            val client = com.wizaicorp.apkfactory.FirebaseAuthManager.getFirebaseGoogleSignInClient(context)
                            client.signOut().addOnCompleteListener {
                                firebaseAuthLauncher.launch(client.signInIntent)
                            }
                        },
                        shape = androidx.compose.foundation.shape.RoundedCornerShape(8.dp),
                        ...
                        Text("Google ile Bağla", fontSize = 11.sp, color = WHITE, fontWeight = androidx.compose.ui.text.font.FontWeight.Bold)
                    }
                } else {
                    androidx.compose.material3.Button(
                        onClick = {
                            val client = com.wizaicorp.apkfactory.FirebaseAuthManager.getFirebaseGoogleSignInClient(context)
                            client.signOut().addOnCompleteListener {
                                prefs.edit().putBoolean("firebase_connected", false).apply()
                                isFirebaseConnected = false
                                status = "Bağlantı kesildi"
                            }
                        },
                        shape = androidx.compose.foundation.shape.RoundedCornerShape(8.dp),
                        colors = androidx.compose.material3.ButtonDefaults.buttonColors(containerColor = SURFACE),
                        contentPadding = PaddingValues(horizontal = 12.dp, vertical = 6.dp),
                        modifier = Modifier.height(32.dp)
                    ) {
                        Text("Bağlantıyı Kes", fontSize = 11.sp, color = GREY)
                    }
                }
            }
        }
